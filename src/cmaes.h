...
namespace libcmaes
{
  ...
  CMASolutions cmaes(FitFunc &func,
    ...
    {
        // initialize the mpi environment (closed in CMASolutions)
        MPI_Init(NULL,NULL);
        ...
        // our mpi rank-based branch
        if (world_rank > 0)
        {
            // the candidate vector worked on by the slave
            std::vector<double> candidate;
            candidate.resize(parameters.dim(),0.0);

            // slaves run jobs here
            while (true)
            {
                // store the status of the message
                MPI_Status status;
                ...
                if (command == 1)
                {
                    // break out of the while loop: we are finished
                    break;
                }

            }

            // have to clear MPI state for this slave
            MPI_Finalize();

            // return dummy cmaSolutions
            return CMASolutions();
        }
        else
        {
            // master branch (rank 0) essentially unchanged
